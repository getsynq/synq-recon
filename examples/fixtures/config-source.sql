-- Source tables for the production-ready reconciliation reference example
-- Simulates an OLTP database (PostgreSQL) with transaction, revenue, customer, and order data

-- 10K transactions for full reconciliation demo
...
-- Regional revenue data for aggregate reconciliation demo
...
-- 5K customers for row count check
...
-- 1K orders for column mapping demo
CREATE OR REPLACE TABLE orders AS
SELECT
  CAST(id AS BIGINT) AS order_id,
  id * 100.0 AS invoice_amount,
  'customer_' || CAST(id AS VARCHAR) AS customer_name
